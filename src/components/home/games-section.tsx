...
import { Gamepad2, Sparkles, Star, ArrowRight, Puzzle, Brain, Shapes, Palette } from 'lucide-react'

const games = [
  {
    title: 'Memory Match', icon: Brain, color: '#FF6B6B', difficulty: 'Easy',
    desc: 'Flip cards and find matching pairs!', players: '1-2',
  },
  {
    title: 'Shape Sorter', icon: Shapes, color: '#FFD93D', difficulty: 'Beginner',
    desc: 'Sort shapes into the right holes!', players: '1',
  },
  {
    title: 'Color Puzzle', icon: Palette, color: '#6BCBFF', difficulty: 'Easy',
    desc: 'Match colors to complete the picture!', players: '1',
  },
  {
    title: 'Number Jump', icon: Puzzle, color: '#6C63FF', difficulty: 'Medium',
    desc: 'Hop on numbers to solve equations!', players: '1-2',
  },
  {
    title: 'Word Builder', icon: Gamepad2, color: '#51CF66', difficulty: 'Medium',
    desc: 'Drag letters to spell words!', players: '1',
  },
  {
    title: 'Quick Math', icon: Brain, color: '#FF6B6B', difficulty: 'Hard',
    desc: 'Solve math problems before time runs out!', players: '1',
  },
]

function GameCard({ title, icon: Icon, color, difficulty, desc, players, index }: {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  title: string; icon: React.ComponentType<any>
  color: string; difficulty: string; desc: string; players: string; index: number
}) {
  return (
    <motion.div
      className="group"
    ...
    >
      ...
      >
        ...
        <div className="p-4 sm:p-5">
          ...
          <p className="text-xs text-white/40 font-nunito mb-3 line-clamp-2">{desc}</p>
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-white/30 font-nunito">👥 {players} players</span>
            <motion.button
